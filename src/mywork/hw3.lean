...
ANSWER HERE:
Plato is a person, and mortality is a characteristic of all
people. Given any arbitrary person, we know they are mortal. Therefore,
Plato is mortal, because all people are mortal.
-/
...
variable Triangle: ∀ (a b c: Person), Likes a b → Likes b c → Jealous a c
...
ANSWER HERE:
If P and Q are propositions, then if P and Q are true, then P is true
and Q is true (and elimination left and right). If Q is true and P is true,
then Q is true or P is true (or introduction left and right).
...
#check
	...
	∃ (someone_B: Person),
	∀ (everyone_B: Person),
-- All people know someone
		(Knows everyone_A someone_A)
-- Who knows someone
	∧ (Knows someone_A someone_B)
-- Who knows everyone
	∧ (Knows someone_B everyone_B)
